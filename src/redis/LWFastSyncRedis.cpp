...
void LWFastSyncRedis::writeString(const std::string& key,
...
std::string LWFastSyncRedis::readWriteString(const std::string& key,
		const std::string& value) {
	redisReply *reply = (redisReply *) redisCommand(c, "GETSET %s %s",
...
void LWFastSyncRedis::writeTimeoutString(const std::string& key,
		const std::string& value, int timeout) {
	timeout = (timeout <= 0) ? -1 : timeout;
	redisReply *reply = (redisReply *) redisCommand(c, "SETEX %s %d %s",
			key.c_str(), timeout, value.c_str());
	printf("SET: %s\n", reply->str);
	freeReplyObject(reply);
}

std::string LWFastSyncRedis::readString(const std::string& key) {
...
void LWFastSyncRedis::writeBinary(const std::string& key, const char* buff,
		int size) {
	const char *v[3];
	size_t vlen[3];
	v[0] = "SET";
...
void LWFastSyncRedis::readBinary(const std::string& key, char* buff, int size) {
	redisReply *reply = (redisReply *) redisCommand(c, "GET %s", key.c_str());
